...
public class ProjectilePoolManager : SingletonBehaviour<ProjectilePoolManager>
{
    ...
    private Dictionary<PROJECTILE_TYPE, ObjectPool<Projectile>> pools;

    protected override void Awake()
    {
        pools = new Dictionary<PROJECTILE_TYPE, ObjectPool<Projectile>>();
    ...
    {
        if (!pools.ContainsKey(skillData.projectileType))
        {
            string prefabPath = skillData.skillPrefabPath;
            pools.Add(skillData.projectileType, new ObjectPool<Projectile>(ResourcesManager.Load<Projectile>(prefabPath), transform));
        }
